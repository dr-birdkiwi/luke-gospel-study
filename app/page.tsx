...
const chapterNotes: StudyNote[] = [
  ...
  { range: '1:46–55', title: '我心尊主为大', scene: '两位妇女在家中相对而立，一个年长，一个年轻；她们都面对身体和未来的巨大改变。马利亚没有把歌唱成私人日记，而是把自己的怀孕放进亚伯拉罕、以色列和世世代代受压者的故事里。', scripture: '马利亚的诗歌颂赞神顾念卑微者，使饥饿的得饱足，并记念对亚伯拉罕的应许。', literal: '赞美不是逃避现实；它重新命名现实。马利亚反复使用强有力的动作描写神：看顾、施展大能、叫有权柄的降卑、叫卑微的升高、叫饥饿的得饱足。', context: '古代敬拜常用诗歌保存群体记忆；路加把这首歌放在个人故事与以色列历史的交界处。它不是温柔的私人抒情，而是对神如何翻转骄傲、权势与匮乏的神学宣告。', connection: '可对照撒上 2:1–10、诗 113:5–9、诗 146:5–9，听见“神抬举卑微者”的旧约回声；再回看路 4:18–19，观察耶稣如何把这条线带入公开事奉。', life: '讨论：我们的敬拜是否只说“我得着了什么”，还是也看见神对弱小者、公义与群体的心意？操练：本周写一段三节的赞美：一节记念神已经做过的事，一节为被忽略的人代求，一节写出你愿意参与的行动。' },
...
function StudyNoteCard({ note, index }: { note: StudyNote; index: number }) { return <article className="study-note"><div className="note-index">{String(index + 1).padStart(2, '0')}</div><div className="note-body"><div className="note-heading"><span className="verse-range">{note.range}</span><h3>{note.title}</h3></div><div className="note-scene"><span className="note-label">进入现场</span><p>{note.scene}</p></div><p className="scripture-line">{note.scripture}</p><div className="note-columns"><div className="note-insight note-observation"><span className="note-label">经文观察</span><p>{note.literal}</p></div><div className="note-insight note-context"><span className="note-label">历史窗口</span><p>{note.context}</p>{note.connection && <><span className="note-sub-label">旧约／新约回声</span><p>{note.connection}</p></>}</div>{note.life && <div className="note-insight note-life"><span className="note-label">信仰生活讨论</span><p>{note.life}</p></div>}</div></div></article>; }
